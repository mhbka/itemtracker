...
import { FloatHardCriterion, HardCriterion, IntHardCriterion } from '@/types/evaluationCriteria'
import { ref } from 'vue'
...
const updateValue = () => {
  let criterionValue: FloatHardCriterion

  if (!operator.value) {
    criterionValue = null
  } else if (operator.value === 'Between') {
    if (value1.value !== null && value2.value !== null) {
      if (value1.value < value2.value) criterionValue = { Between: [value1.value, value2.value] }
